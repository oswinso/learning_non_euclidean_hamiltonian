...
void EulerAngles2RotMat_xzx(Matrix<ld> &R, ld phi, ld theta, ld psi) {
  R = RotX(phi) * RotZ(theta) * RotX(psi);
}

void EulerAngles2RotMat_zxz(Matrix<ld> &R, ld phi, ld theta, ld psi) {
...
void EulerAngles2RotMat(Matrix<ld> &R, ld phi, ld theta, ld psi,
  ...
  switch (convention) {
  case EulerConvention::XZX:
    return EulerAngles2RotMat_xzx(R, phi, theta, psi);
  case EulerConvention::ZXZ:
    return EulerAngles2RotMat_zxz(R, phi, theta, psi);
  }
  throw std::runtime_error("Switch failed lol");
}
...
void EulerAngles2RotMat(Matrix<ld> &R, const Vec3<ld> &euler_angle,
  ...
  EulerAngles2RotMat(R, euler_angle.GetElement(1), euler_angle.GetElement(2),
                     euler_angle.GetElement(3), convention);
}
