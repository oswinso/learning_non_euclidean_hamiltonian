...
Body::Body(const json &j, EulerConvention convention)
    : name(j.value<std::string>("name", "")),
      mass(j.value<ld>("mass", -1)), radius{},
  ...
  } else if (j.contains("diameter")) {
    ld diameter = j.value<ld>("diameter", -1) / kAU * 1000;
    radius = diameter / ld(2.0);
  } else {
    throw std::runtime_error("No radius or diameter specified!");
  }
  ...
  if (true || rigid) {
    ...
    if (j.contains("oblateness")) {
      ...
      std::cout << "axis_a,b,c = " << axis_a << ", " << axis_b << ", " << axis_c
                << std::endl;
      ...
      ld oblateness =
          1 - std::pow(radius / axes[0] * (kAU / ld(1000)), ld(3.0));
      fmt::print("Equiv oblateness={}\n", oblateness);

      ld axis_a1 = (axes[0] / kAU) * ld(1000);
      fmt::print("axis_a1={}\n", axis_a1);

      ld axis_a2 = radius / pow(1 - oblateness, ld(1.0) / ld(3.0));
      fmt::print("axis_a2={}\n", axis_a2);
    ...
    if (j.contains("_R")) {
      ...
      // Check which convention is used - either ZXZ or XZX. Earth-sun uses XZX,
      // while TRAPPIST uses ZXZ.
...
std::istream &operator>>(std::istream &is, Body &s) {
  ...
  if (s.rigid) {
    ...
    is >> name >> mass >> radius >> s.q >> velocity >> ang_vel >> euler_angle >>
        s.symmetric >> s.I1 >> s.I2 >> s.I3;
    s.pi = s.I * ang_vel;
    EulerAngles2RotMat(s.R, euler_angle, EulerConvention::XZX);
  } else {
