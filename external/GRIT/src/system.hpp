...
struct OutputFormat {
  ...
  bool velocity = true;
  bool rotmat = true;
  bool lin_momentum = true;
  bool ang_momentum = true;
  bool axis = true;
  bool spin = true;
  bool obliquity = true;
...
private:
  ...
  // Keplerian splitting: H = H_Kep + H_Sun + H_interaction (Duncan et. al.
  // 1998)
  ...
  Vec3<ld>
  PVPq_rigid(unsigned int i,
             bool V2 = false) const; // $\frac{\partial{V}}{\partial{q}}$
  Vec3<ld> PVPq_pt(unsigned int i,
                   bool V2 = false) const; // $\frac{\partial{V}}{\partial{q}}$
  ...
  Vec3<ld> PV1Pq(unsigned int i) const; // $\frac{\partial{V1}}{\partial{q}}$
