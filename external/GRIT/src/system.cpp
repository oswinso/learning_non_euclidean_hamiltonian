...
void System::InitFromFile(const std::string &dir) {
  ...
  try {
    ...
    std::cerr << "Invalid json format. Check your input file." << ex.byte
              << std::endl;
  }
...
void System::InitSystem(const json &j) {
  ...
  EulerConvention convention = EulerConvention::XZX;
  if (j.contains("euler_convention")) {
    auto convention_str = j["euler_convention"].get<std::string>();
    if (convention_str == "XZX") {
      convention = EulerConvention::XZX;
      std::cout << "Using convention XZX (Sun-Earth)!\n";
    } else if (convention_str == "ZXZ") {
      convention = EulerConvention::ZXZ;
      std::cout << "Using convention ZXZ (TRAPPIST)!\n";
    } else {
      throw std::runtime_error("Unknown convention_str!");
    }
  }
  ...
  for (json::iterator it = jbody.begin(); it != jbody.end(); it++) {
    Body new_body{it.value(), convention};
    AddBody(new_body);
  }
  ...
  fmt::print("external_force: {}, symmetric: {}\n", external_force, symmetric);

  // Dump the inertia and mass to full precision.
  {
    std::ofstream f{sys_dir + "/data_in_mat/inertias.txt"};
    for (auto &body : bodies) {
      f << fmt::format("{}\n", body.name);
      f << fmt::format("{}\n", body.mass);
      f << fmt::format("[{}, {}, {}]\n", body.I1, body.I2, body.I3);
      f << "\n";
    }
    fmt::print("Saved inertia + mass to inertias.txt!\n");
  }
...
template <typename T> std::string tostr(const T &arr) {
  return fmt::format("[{:12.5e}]", fmt::join(arr, ", "));
}
...
void System::T2(int steps) {
  ...
  for (const auto &body : bodies) {
    ...
    fmt::print("    q: {}, p: {}\n"
               "    R: {}\n"
               "   Pi: {}\n",
               tostr(body.q), tostr(body.p), tostr(body.R), tostr(body.pi));
  }
  ...
  for (int current_step = 0; current_step < steps; current_step++) {
    ...
    if (!external_force && symmetric)
      // No external forces, Symmetric.
      ABSchemeB(h);
    else if (external_force && symmetric) {
      // External forces, Symmetric.
      ABSchemeB(h / ld(2.0));
      StepwiseOperations();
      ABSchemeB(h / ld(2.0));
    } else if (!external_force && !symmetric) {
      // No external forces, Not symmetric
      ABSchemeB(h / ld(2.0));
      SchemeE(h);
      ABSchemeB(h / ld(2.0));
    } else {
      // External Forces, Not Symmetric
      ABSchemeB(h / ld(2.0));
      SchemeE(h / ld(2.0));
      StepwiseOperations();
      SchemeE(h / ld(2.0));
...
void System::ABSchemeA(ld h) {
  ...
  for (auto i = 0u; i < bodies.size(); i++) {
    if (true || bodies[i].rigid) {
      ...
      fmt::print("    Body {} - theta={:12.4e}, gamma={:12.4e}, Pi={}\n"
                 "               Rpi={}\n"
                 "                Rz={}\n",
                 i, theta, gamma, tostr(bodies[i].pi), tostr(Rpi), tostr(Rz));
...
void System::ABSchemeB(ld h) {
  ...
  for (auto i = 0u; i < bodies.size(); i++) {
    if (compensated_summation) {
      ...
      auto pvpq = PVPq(i);
      fmt::print("Body {} PVPq: {}\n", i, pvpq);
      //      std::cout << "Body " << i << " PVPq: " << pvpq << "\n";
      bodies[i].p -= pvpq * h;
    }

    if (bodies[i].rigid) {
      const auto pvpr = PVPR(i);

      fmt::print("Body {} pvpr: {}\n", i, pvpr);

      const auto dPi = Rot(bodies[i].R.Transpose() * pvpr) * -1;
      bodies[i].pi += dPi * h;
  ...
  for (auto i = 0u; i < bodies.size(); i++) {
    if (bodies[i].rigid) {
      ...
      fmt::print("Body {} - R={}\n"
                 "         I={}\n"
                 "       RIR={}\n",
                 i, tostr(bodies[i].R), tostr(bodies[i].I), tostr(RIR[i]));
    }
  }
}

Vec3<ld> System::PVPq_rigid(unsigned int i, bool V2) const {
  Vec3<ld> ret(ld(0.0));
  for (unsigned int j = 0u; j < bodies.size(); j++) {
    if (j != i) {
      ...
      ld tmp = 0.0;
      if (!V2) {
        tmp = bodies[i].mass * bodies[j].mass / div3;
      }
      fmt::print("    Body {} - tmp={}\n", i, tmp);
      ...
      if (bodies[j].rigid) {
        ...
        tmp -=
            qi_qj.DotProduct(
                (bodies[j].mass * RIR[i] + bodies[i].mass * RIR[j]) * (qi_qj)) *
            (ld(7.5) / div7);

        fmt::print("    Body {} - tmp={}, RIR[i]={}, RIR[j]={}\n", i, tmp,
                   tostr(RIR[i]), tostr(RIR[j]));
...
Vec3<ld> System::PVPq_pt(unsigned int i, bool V2) const {
  Vec3<ld> ret(ld(0.0));
...
Vec3<ld> System::PVPq(unsigned int i, bool V2) const {
  ...
  if (V2) {
    throw std::runtime_error("V2 should be False.");
  }
  ...
  if (parameters.potential_order == 4) {
    throw std::runtime_error("potential_order == 4");
    ret += PVPq4thOrder(i);
  }
  return ret;
}
...
Vec3<ld> System::PV2Pq(unsigned int i) const { return PVPq(i, true); }
  ...
  if (bodies[i].rigid) {
    for (auto j = 0u; j < bodies.size(); j++) {
      if (j != i) {
        ...
        //                  bodies[j].mass
        //                  << ", div5=" << div5 << ", qqT=["
        //                  << (qi_qj * qi_qj.Transpose()) << "]";
      }
    }
    //    fmt::print("\ni={}, ret1=[{:11.5e}]", i, fmt::join(ret, " "));
    //    fmt::print("\n     R=[{:13.7e}], I=[{:13.7e}]", fmt::join(bodies[i].R,
    //    " "),
    //               fmt::join(bodies[i].I, " "));
    //    std::cout << "\ni=" << i << ", ret1=[" << ret << "]";
    auto fkfk = bodies[i].R * bodies[i].I;
    //    fmt::print("\n     fkfk=[{:11.5e}]", fmt::join(fkfk, " "));
    auto fku = ret * fkfk;
    //    fmt::print("\n     fku=[{:11.5e}]", fmt::join(fku, " "));

    ret = kG * fku;
    //    std::cout << "\n    ret2=[" << ret << "]";
    //    fmt::print("\n     ret2=[{:11.5e}]", fmt::join(ret, " "));
    //    fmt::print("\n     kG={:.20f}", kG);
    //    if (parameters.potential_order == 4) {
    //      ret += PVPR4thOrder(i);
    //    }
        ...
        if (j != i && (bodies[i].rigid || bodies[j].rigid)) {
          ...
          auto tmp_mat = Ri.Transpose() * Rj *
                         ...
                         Matrix<ld>(std::vector<ld>{rai2 / 5, 0, 0, 0, rbi2 / 5,
                                                    0, 0, 0, rci2 / 5},
                                    3, 3);
          E -= bodies[i].mass * bodies[j].mass * 3.0 / 4.0 / div5 *
               tmp_mat.Trace();
          ...
          tmp_mat = qi_qj2 * Ri *
                    Matrix<ld>(std::vector<ld>{rai2 * rai2 * 3 / 35, 0, 0, 0,
                               ...
                               3, 3) *
                    Ri.Transpose() * qi_qj2;
          E -= bodies[i].mass * bodies[j].mass * 35.0 / 8.0 / div9 *
               tmp_mat.Trace();
...
json System::OutputDataJson() const {
  ...
  for (unsigned int idx = 0; idx < bodies.size(); idx++) {
    ...
    if (output_format.rotmat) {
      j["rotmat"] = bodies[idx].R.mat();
    }
    if (output_format.rotmat) {
      j["lin_momentum"] = bodies[idx].p;
    }
    if (output_format.rotmat) {
      j["ang_momentum"] = bodies[idx].pi;
    }
...
void System::InitMatFile() const {
  ...
  for (unsigned int idx = 0; idx < GetNum(); idx++) {
    ...
    if (output_format.velocity)
      body_files[idx] << std::setw(30) << "u" << std::setw(30) << "v"
                      << std::setw(30) << "w";
    if (output_format.rotmat) {
      body_files[idx] << std::setw(30) << "r11" << std::setw(30) << "r12"
                      << std::setw(30) << "r13" << std::setw(30) << "r21"
                      << std::setw(30) << "r22" << std::setw(30) << "r23"
                      << std::setw(30) << "r31" << std::setw(30) << "r32"
                      << std::setw(30) << "r33";
    }
    if (output_format.lin_momentum) {
      body_files[idx] << std::setw(30) << "p1" << std::setw(30) << "p2"
                      << std::setw(30) << "p3";
    }
    if (output_format.ang_momentum) {
      body_files[idx] << std::setw(30) << "pi1" << std::setw(30) << "pi2"
                      << std::setw(30) << "pi3";
    }
  ...
  if (output_format.Hamiltonian) {
    ...
    // Print the header line.
    Hamiltonian_file << std::setw(30) << "time" << std::setw(30)
...
void System::OutputBodyData(const json &j,
  ...
  for (unsigned int idx = 0; idx < fouts.size(); idx++) {
    fouts[idx] << std::setprecision(std::numeric_limits<double>::digits10 + 1)
               << std::scientific << std::left << std::setw(30)
               << double(j["time"]);
    ...
    if (output_format.rotmat) {
      auto rotmat = body_json["rotmat"];
      for (json::iterator it = rotmat.begin(); it != rotmat.end(); it++) {
        fouts[idx] << std::setw(30) << double(it.value());
      }
    }

    if (output_format.lin_momentum) {
      auto lin_mom = body_json["lin_momentum"];
      for (json::iterator it = lin_mom.begin(); it != lin_mom.end(); it++) {
        fouts[idx] << std::setw(30) << double(it.value());
      }
    }

    if (output_format.ang_momentum) {
      auto ang_mom = body_json["ang_momentum"];
      for (json::iterator it = ang_mom.begin(); it != ang_mom.end(); it++) {
        fouts[idx] << std::setw(30) << double(it.value());
      }
    }
