...
class span {
 ...
 private:
    void _range_check(size_t i) const {
        if (i >= this->size()) {
            throw std::out_of_range(
                fmt::format("__n (which is {}) >= this->size() (which is {})", i, this->size()));
        }
    }
...
struct NpyArray {
    NpyArray(std::vector<size_t> _shape, size_t _word_size, std::string dtype, bool _fortran_order)
    ...
    span<const T> as_span() const {
        return span<const T>{data<T>(), num_vals};
    }
...
void npy_save(const std::string &fname,
    ...
    if (fp) {
        ...
        bool fortran_order;
        std::string dtype;
        parse_npy_header(fp, word_size, true_data_shape, dtype, fortran_order);
    ...
    if (fp == nullptr) {
        throw std::runtime_error(fmt::format("fopen({}) failed!", fname));
    }
